...
std::vector<PathPoint> planCircularPath(const PathPoint& start, const PathPoint& end, const double radius, const double stepSize);
//直线运动规划
// void linear_move();
...
//测试直线运动，与上面的区别是：先存储所有坐标点，然后再运动
void linear_move_test();
//测试直线运动，与上面的区别是：先存储所有坐标点，然后再运动
void interpolate_test(Point startPoint, Point endPoint, float stepSize);

void linear_interpolation(double x1, double y1, double z1, double x2, double y2, double z2, int n);
double distance(double x1, double y1, double z1, double x2, double y2, double z2);
void forward_kinematics(double q1, double q2, double q3, double q4, double q5, double q6, double &x, double &y, double &z);



Point splineInterpolate(const Point& startPoint, const Point& endPoint, double t);

std::vector<Point> generateLinearPath(const Point& startPoint, const Point& endPoint, int numSteps);
